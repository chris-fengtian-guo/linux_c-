...
void handle_write(const boost::system::error_code& error, std::size_t bytes_transferred, std::size_t total_bytes) {
    if (error) {
        std::cerr << "Failed to send file: " << error.message() << "\n";
    } else {
...
void send_file(tcp::socket& socket, const std::string& file_name) {
    ...
    socket.async_write_some(boost::asio::buffer(file_data), 
        std::bind(handle_write, std::placeholders::_1, std::placeholders::_2, size));
}
